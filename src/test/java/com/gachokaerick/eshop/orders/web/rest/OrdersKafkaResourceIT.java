...
import com.gachokaerick.eshop.orders.config.KafkaProperties;
...
class OrdersKafkaResourceIT {
    ...
    void setup() {
        ...
        OrdersKafkaResource kafkaResource = new OrdersKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/orders-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/orders-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
